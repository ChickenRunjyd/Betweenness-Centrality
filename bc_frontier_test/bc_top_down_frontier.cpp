/**
 File name: bc_top_down_frontier.cu
 Author: Yuede Ji
    ...
    while(flag)
    {
        ...
        for(index_t j=0; j<g->vert_count; ++j)
        {
            ...
            for(index_t i=g->beg_pos[j]; i<g->beg_pos[j+1]; ++i)
            {
                index_t v = g->csr[i];
                //printf("weight[%u] = %g\n", v, g->weight[v]);
                if(dist[0][v] > dist[0][j] + g->weight[i])
                {
                    dist[0][v] = dist[0][j] + g->weight[i];
                    flag = true;
                    sa[0][v] = level + 1;
                    sp_count[0][v] = 0;
                }
        ...
        for(index_t j=0; j<g->vert_count; ++j)
        {
            ...
            for(index_t i=g->beg_pos[j]; i<g->beg_pos[j+1]; ++i)
            {
                index_t v = g->csr[i];
                //printf("weight[%u] = %g\n", v, g->weight[v]);
                if(dist[1][v] > dist[1][j] + g->weight[i])
                {
                    dist[1][v] = dist[1][j] + g->weight[i];
                    flag = true;
                    sa[1][v] = level + 1;
                    sp_count[1][v] = 0;
                }
        ...
        for(index_t i=0; i<g->vert_count; ++i)
        {
            if(sa[0][i] == cur)
            {
                ...
                for(index_t j=g->beg_pos[w]; j<g->beg_pos[w+1]; ++j)
                {
                    index_t v = g->csr[j];
                    //printf("v = %u\n", v);
                    if(dist[0][w] == dist[0][v] + g->weight[j])
                    {
                        if(sp_count[0][w] != 0)
        ...
        for(index_t i=0; i<g->vert_count; ++i)
        {
            if(sa[1][i] == cur)
            {
                ...
                for(index_t j=g->beg_pos[w]; j<g->beg_pos[w+1]; ++j)
                {
                    index_t v = g->csr[j];
                    //printf("v = %u\n", v);
                    if(dist[1][w] == dist[1][v] + g->weight[j])
                    {
                        if(sp_count[1][w] != 0)
...
{
    printf("Input: ./bc /path/to/beg /path/to/csr /path/to/weight vertex-count\n");
